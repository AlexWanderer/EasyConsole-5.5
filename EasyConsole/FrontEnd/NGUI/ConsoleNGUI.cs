...
class ConsoleNGUI : MonoBehaviour
{
    ...
    void Update()
    {
        if (isOpen)
        {
            ...
            int historyScrollValue = Mathf.Clamp((int)((lines.Count() - linesVisible) * (1 - logScroll.value)), 0, lines.Count());
            ...
            int newObjectPanelScroll = Mathf.Clamp((int)((displayObjects.Length - objectPanelButtonVisibleCount) *
                ...
                (objectPanelScroll.value)), 0, displayObjects.Length);
            ...
            int newComponentPanelScroll = Mathf.Clamp((int)((displayComponents.Length - componentPanelButtonVisibleCount) *
                ...
                (componentPanelScroll.value)), 0, displayComponents.Length);
            ...
            int newMethodPanelScroll = Mathf.Clamp((int)((displayMethods.Length - methodPanelButtonVisibleCount) * //(methodPanelScroll.scrollValue)), 0, displayMethods.Length);
                (methodPanelScroll.value)), 0, displayMethods.Length);
    ...
    void OnSubmit()
    {
        ...
        //commandInput.text = "";
        string command = commandInput.value;
        commandInput.value = "";
    ...
    void OnMethodButtonClick(GameObject go)
    {
        ...
        commandInput.value = commandInput.value.Substring(0, commandInput.value.IndexOf('.', firstDot + 1));
        commandInput.value += "." + go.GetComponentInChildren<UILabel>().text;
    }
    ...
    void OnComponentButtonClick(GameObject go)
    {
        ...
        commandInput.value += "." + go.GetComponentInChildren<UILabel>().text + ".";
        if (firstDot > 0)
            commandInput.value = commandInput.value.Substring(0, firstDot);
        if (commandInput.value.EndsWith("/"))
            commandInput.value = commandInput.value.Substring(0, command.Length - 1);
    ...
    void OnObjectButtonClick(GameObject go)
    {
        ...
        if (commandInput.value.LastIndexOf('/') >= 0)
            commandInput.value = commandInput.value.Substring(0, command.LastIndexOf('/'));
        commandInput.value += "/" + go.GetComponentInChildren<UILabel>().text + "/";
    }
}
